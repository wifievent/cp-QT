...
void WERawClient::setSocketOpt(GIp myip, int port) {
...
void WERawClient::setIpHeader(GIpHdr iphdr) {
    ...
    iph_->check = 0;
    iph_->saddr = iphdr.sip_;
    sip_ = QString(iphdr.sip());
    qDebug() << "This packet is from here:" << sip_;
...
void WERawClient::setTcpHeader(GTcpHdr tcphdr) {
    ...
    tcph_->fin = tcphdr.flags_ & 0x01;
    tcph_->syn = tcphdr.flags_ >> 1 & 0x01;
    tcph_->rst = tcphdr.flags_ >> 2 & 0x01;
    tcph_->psh = tcphdr.flags_ >> 3 & 0x01;
    tcph_->ack = tcphdr.flags_ >> 4 & 0x01;
    tcph_->urg = tcphdr.flags_ >> 5 & 0x01;
...
void WERawClient::send() {
    qDebug() << "Sending data to tls server data is down below...";
